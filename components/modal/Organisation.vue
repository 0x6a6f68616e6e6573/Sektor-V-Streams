  ...
  methods: {
    async submit() {
      let response = await this.$axios.post('/api/', {
        query: `mutation createOrganisation($name: String!, $color: String!, $description: String!, $group: String!) {
          createOrganisation(input: {name: $name, color: $color, description: $description, group: $group}) {
            name
          }
        }`,
        variables: { ...this.form, group: this.group },
      });
      this.fetch();
